#ifndef GENERIC_TREL_HPP
#define GENERIC_TREL_HPP 1
...
struct earlier_bmin {
    inline bool operator() (shared_ptr<gen_zone> &z1, shared_ptr<gen_zone> &z2){
...
static vector<shared_ptr<gen_zone>> gen_filter(const vector<shared_ptr<gen_zone>> &zs){

    vector<shared_ptr<gen_zone>> active, active_temp;
    vector<shared_ptr<gen_zone>> result;

    for(auto z1it = zs.cbegin(); z1it != zs.cend(); z1it++){

        bool already_included = std::any_of(active.begin(), active.end(), 
            [z1it](shared_ptr<gen_zone> &z2){return z2->includes(*z1it);});

        if(!already_included){

            active.erase( std::remove_if(active.begin(), active.end(), 
                [z1it](shared_ptr<gen_zone> &z2){return (*z1it)->includes(z2);}), active.end());
...
static bool gen_includes(const vector<shared_ptr<gen_zone>> &zs1, const vector<shared_ptr<gen_zone>> &zs2){
    ...
    vector<shared_ptr<gen_zone>> act_1;
    ...
    while(it1 != zs1.cend() and it2 != zs2.cend()) {
        ...
        if ((*it1)->compare_less_bmin_bmax(*it2)){ //  z1.bmin < z2.bmax
            ...
            [&](shared_ptr<gen_zone> z1){return z1->compare_less_bmax_bmin(*it2);}), act_1.end()); // remove if z1.bmax < z2.bmin
            bool z2_incd = std::any_of(act_1.begin(), act_1.end(), 
                [&](shared_ptr<gen_zone> z1){return z1->includes(*it2);});
            if(!z2_incd){
                return false;
            }
            it2++;
...
static vector<shared_ptr<gen_zone>> gen_intersection(const vector<shared_ptr<gen_zone>> &zs1,
    const vector<shared_ptr<gen_zone>> &zs2){

    vector<shared_ptr<gen_zone>> result;

    vector<shared_ptr<gen_zone>> act_1, act_2, act_r, act_r_temp;
    ...
    while(it1 != zs1.cend() and it2 != zs2.cend()) {
        ...
        if ((*it1)->compare_less_bmin(*it2)){
            ...
            act_2.erase(std::remove_if(act_2.begin(), act_2.end(), 
                [&](shared_ptr<gen_zone> z2){return z2->compare_less_bmax_bmin(*it1);}), act_2.end());
            ...
            for(const auto& z2 : act_2){
                ...
                if( kid->is_nonempty() and 
                    !std::any_of(act_r.begin(), act_r.end(), 
                        [&kid](shared_ptr<gen_zone> &zr){return zr->includes(kid);} ))
                {
                    act_r.erase( std::remove_if(act_r.begin(), act_r.end(), 
                        [&kid](shared_ptr<gen_zone> &zr){return kid->includes(zr);}), act_r.end());
                    act_r.push_back(kid);
            ...
            act_2.push_back(*it2);
            act_1.erase(std::remove_if(act_1.begin(), act_1.end(), 
                [&](shared_ptr<gen_zone> z1){return z1->compare_less_bmax_bmin(*it2);}), act_1.end()); // remove if z1.emax < z2.bmin
...
static vector<shared_ptr<gen_zone>> gen_concatenation(const vector<shared_ptr<gen_zone>> &_zs1, 
    const vector<shared_ptr<gen_zone>> &zs2){
    ...
    // Could be better?
    vector<shared_ptr<gen_zone>> zs1;
    for(auto zs1t : _zs1){
    ...
    while(it1 != zs1.cend() and it2 != zs2.cend()) {
        ...
        if ((*it1)->compare_less_emin_bmin(*it2)){
            act_1.push_back(*it1);
            act_2.erase(std::remove_if(act_2.begin(), act_2.end(), 
                [&](shared_ptr<gen_zone> z2){return z2->compare_less_bmax_emin(*it1);}), act_2.end());
            ...
            act_2.push_back(*it2);
            act_1.erase(std::remove_if(act_1.begin(), act_1.end(), 
                [&](shared_ptr<gen_zone> z1){return z1->compare_less_emax_bmin(*it2);}), act_1.end()); // remove if z1.emax < z2.bmin
...
static vector<shared_ptr<gen_zone>> gen_union(const vector<shared_ptr<gen_zone>> &zs1, 
    const vector<shared_ptr<gen_zone>> &zs2){
    vector<shared_ptr<gen_zone>> result;
...
static vector<shared_ptr<gen_zone>> gen_transitive_closure(const vector<shared_ptr<gen_zone>> &zs){
    vector<shared_ptr<gen_zone>> result;

    vector<shared_ptr<gen_zone>> zplus = zs;
    vector<shared_ptr<gen_zone>> zlast = zs;

    vector<shared_ptr<gen_zone>> znext;
